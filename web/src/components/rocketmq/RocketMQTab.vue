<template>
  <div class="mw-tab">
    ...
    <div v-if="!loading && instances.length === 0" class="empty-guide glass">
      ...
      <p class="empty-desc">当前没有已采集的 RocketMQ 实例。请在运行 Agent 的节点上配置 Broker 地址。</p>
    ...
    <template v-if="instances.length > 0">
      ...
      <div class="mw-list glass">
        ...
        <el-table class="rocketmq-table" :data="pagedInstances" style="width: 100%" @row-click="openDetail" :row-class-name="rowClass" size="small" stripe @sort-change="onSortChange">
          <el-table-column prop="group" label="分组" min-width="95" />
          ...
          <el-table-column prop="consumerGroupCount" label="消费组" min-width="95" sortable />
          <el-table-column prop="consumerLag" label="消费延迟" min-width="100" sortable />
          <el-table-column prop="producerCount" label="生产者" min-width="90" sortable />
          <el-table-column prop="consumerCount" label="消费者" min-width="90" sortable />
        </el-table>
        <div class="pager">
    ...
    <el-drawer v-model="drawerVisible" :title="detailTitle" size="50%" :destroy-on-close="true">
      <div v-if="selected" class="detail-content">
        ...
        <div class="metric-grid">
          ...
          <div class="metric-cell"><div class="mc-label">消费组</div><div class="mc-value">{{ selected.consumerGroupCount }}</div></div>
          <div class="metric-cell"><div class="mc-label">消费延迟</div><div class="mc-value">{{ formatNum(selected.consumerLag) }}</div></div>
          <div class="metric-cell"><div class="mc-label">生产者</div><div class="mc-value">{{ selected.producerCount }}</div></div>
          <div class="metric-cell"><div class="mc-label">消费者</div><div class="mc-value">{{ selected.consumerCount }}</div></div>
          <div class="metric-cell"><div class="mc-label">消息积压</div><div class="mc-value" :class="{'metric-bad': selected.consumerLag > 0}">{{ formatNum(selected.consumerLag) }}</div></div>
...
async function loadTrendChart(row) {
  ...
  try {
    const lagData = await http.get(`/api/v1/query/range?node=${row.node}&metric=rocketmq_consumer_lag&start=${start}&end=${end}&step=60`)
    const series = []
...
.rocketmq-table :deep(th) { white-space: nowrap; }
.rocketmq-table :deep(td) { white-space: nowrap; }
