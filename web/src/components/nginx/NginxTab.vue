<template>
  <div class="mw-tab">
    ...
    <div v-if="!loading && instances.length === 0" class="empty-guide glass">
      ...
      <p class="empty-desc">当前没有已采集的 Nginx 实例。请在运行 Agent 的节点上配置 Nginx stub_status 地址。</p>
    ...
    <template v-if="instances.length > 0">
      ...
      <div class="chart-section glass">
        ...
        <el-table :data="instances" style="width: 100%" @row-click="openDetail" :row-class-name="rowClass">
          ...
          <el-table-column prop="accepts" label="接受连接" width="100" sortable />
          <el-table-column prop="handled" label="处理连接" width="100" sortable />
          <el-table-column prop="requests" label="总请求" width="100" sortable />
          <el-table-column prop="reading" label="Reading" width="80" />
          <el-table-column prop="writing" label="Writing" width="80" />
          <el-table-column prop="waiting" label="Waiting" width="80" />
        </el-table>
      </div>
    </template>
    ...
    <el-drawer v-model="drawerVisible" :title="detailTitle" size="50%" :destroy-on-close="true">
      <div v-if="selected" class="detail-content">
        ...
        <div class="metric-grid">
          <div class="metric-cell"><div class="mc-label">活跃连接</div><div class="mc-value">{{ selected.activeConnections }}</div></div>
          <div class="metric-cell"><div class="mc-label">接受连接</div><div class="mc-value">{{ formatNum(selected.accepts) }}</div></div>
          <div class="metric-cell"><div class="mc-label">处理连接</div><div class="mc-value">{{ formatNum(selected.handled) }}</div></div>
          <div class="metric-cell"><div class="mc-label">总请求</div><div class="mc-value">{{ formatNum(selected.requests) }}</div></div>
          <div class="metric-cell"><div class="mc-label">Reading</div><div class="mc-value">{{ selected.reading }}</div></div>
          <div class="metric-cell"><div class="mc-label">Writing</div><div class="mc-value">{{ selected.writing }}</div></div>
          <div class="metric-cell"><div class="mc-label">Waiting</div><div class="mc-value">{{ selected.waiting }}</div></div>
          <div class="metric-cell"><div class="mc-label">丢弃率</div><div class="mc-value">{{ selected.connectionDropRate ? selected.connectionDropRate.toFixed(2) + '%' : '-' }}</div></div>
        </div>
...
async function loadTrendChart(row) {
  ...
  try {
    const activeData = await http.get(`/api/v1/query/range?node=${row.node}&metric=nginx_active_connections&start=${start}&end=${end}&step=60`)
    const series = []
    if (activeData.series) for (const s of activeData.series) {
      if (s.labels?.instance === row.instance) series.push({ name: '活跃连接', type: 'line', data: s.points.map(p => [p.timestamp, p.value]), smooth: true, areaStyle: { opacity: 0.2 } })
    }
    chartInstance.setOption({
      tooltip: { trigger: 'axis' },
      legend: { data: ['活跃连接'], textStyle: { color: '#8b949e' } },
