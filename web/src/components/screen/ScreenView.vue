    ...
    <el-drawer v-model="settingOpen" title="大屏模块设置" size="300px" :append-to-body="true">
      <div class="cfg-list">
        ...
        <div class="cfg-item">
          ...
          <el-select v-model="cfg.deployLocation" size="small" filterable clearable placeholder="自动探测" style="width: 130px">
            <el-option v-for="p in provinceOptions" :key="p" :value="p" :label="p" />
          </el-select>
        </div>
        <div class="cfg-tip">网络地图上标注的数据中心位置。留空由 server 自动探测；仅当探测失败（如纯内网无公网 IP）时才手动指定省级行政区。</div>
      </div>
      <template #footer>
